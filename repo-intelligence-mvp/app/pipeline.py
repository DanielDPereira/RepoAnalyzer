...
# Etapas do pipeline, na ordem em que ocorrem, com rótulo amigável em
# português. Usado para montar o "stepper" de progresso exibido ao usuário
# (stage_index/stage_count/stage_label) sem duplicar essa lista no front-end.
STAGES = [
...
class Analyzer:
    ...
    # ------------------------------------------------------------------
    # Progresso / métricas em tempo real
    # ------------------------------------------------------------------
    ...
    def _push(self, run_id, **field_updates):
        """Atualiza campos do estado da execução e recalcula a fotografia
        de métricas (stats) exposta pela API. Ponto único de mutação do
        RunState — chamado tanto pela thread principal do pipeline quanto
        pelas threads de análise de arquivos (quando OLLAMA_CONCURRENCY>1),
        por isso protegido por lock."""
        with self.lock:
    ...
    def _estimate_llm_calls(self, files) -> int:
        """Estimativa aproximada do total de chamadas ao modelo, usada
        somente para calcular o tempo estimado restante (ETA). Baseia-se no
        tamanho dos arquivos (bytes) como proxy do número de blocos, já que
        o texto real só é lido durante o processamento."""
        max_chunk_chars = max(1, self.settings.max_chunk_chars)
        total_calls = 1  # síntese final do projeto
        for info in files:
    ...
    def _analyze_files(self, run_id: str, repo_dir: Path, files) -> list[str]:
        ...
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            try:
                ...
                # Interrompe o quanto antes: cancela tudo que ainda não
                # começou a rodar (tarefas já em andamento terminam, pois
                # não há como interromper uma chamada HTTP em curso com
                # segurança). Isso preserva o comportamento de "para no
                # primeiro erro" mesmo usando um executor.
    ...
    def _process_file(self, run_id: str, repo_dir: Path, info) -> str:
        ...
        try:
            ...
            if len(chunks) == 1:
                # Otimização: arquivos que cabem em um único bloco (a
                # maioria de um repositório típico) recebem a análise
                # consolidada diretamente em UMA chamada ao modelo, em vez
                # de uma chamada por bloco + uma chamada de consolidação.
    ...
    def _normalize_github_url(url: str) -> str | None:
        """Aceita variações comuns com que usuários colam URLs do GitHub
        (com/sem www, sufixo .git, barra final, query string, fragmento) e
        devolve uma URL de clone canônica, ou None se inválida."""
        try:
        ...
        host = parts.netloc.lower().split("@")[-1]  # descarta user:pass@ se houver
    ...
    def _budget_summaries(summaries: list[str], max_chars: int) -> tuple[list[str], int]:
        """Mantém o máximo de resumos de arquivo possível dentro de um
        orçamento de caracteres, para não estourar a janela de contexto do
        modelo na etapa de síntese final do projeto. Retorna os resumos
        selecionados e a contagem de quantos ficaram de fora."""
        if max_chars <= 0:
            return summaries, 0
